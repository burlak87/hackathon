...
const app = express()
const port = 3000
app.use(express.json({ limit: '10mb' })) // Для больших массивов
// Конфигурация
...
// Limit concurrency (10 параллельных, чтобы не overload HF)
const limit = pLimit(10)
// Функция для извлечения текста новости (строка или {title, content})
function getNewsText(newsItem) {
	if (typeof newsItem === 'string') return newsItem
	return (newsItem.title || '') + ' ' + (newsItem.content || '')
}
// Косинусное сходство (улучшенное, с проверкой dims)
...
// Fallback: TF-IDF сходство (батчит все заранее)
let globalTfIdf = null; // Глобальный для батчинга
function initTfIdf(allTexts) {
  globalTfIdf = new TfIdf();
  allTexts.forEach(text => globalTfIdf.addDocument(text.toLowerCase().split(/\s+/))); // Простая токенизация
}
...
async function getBatchEmbeddings(texts) {
  ...
  try {
    const response = await apiClient.post(`${HF_API_URL}/${EMBEDDING_MODEL}`, { inputs: texts });
...
async function summarizeText(text) {
  return limit(async () => {
    try {
      const response = await apiClient.post(`${HF_API_URL}/${SUMMARIZATION_MODEL}`, {
        inputs: text,
      ...
      let fallback = sentences.slice(0, MAX_SUMMARY_SENTENCES).join(' ').trim();
...
app.listen(port, () => {
  ...
  console.log('Use POST /process-news with {news: [...], categories: [...], previousEmbeddings: [[...]]}');
});